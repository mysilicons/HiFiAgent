...
`hifi-agent assemble sample.yaml --resume` 只恢复同一个 immutable run。配置、CLI override、输入、环境
manifest、比较策略或 RAG snapshot 漂移时拒绝继续。
...
恢复前先确认没有另一个 live writer。`run.lock` 只允许显式 resume 接管 stale lock。若提示 Nextflow
