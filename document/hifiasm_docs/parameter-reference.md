...
**`-o <FILE=hifiasm.asm>`**
Prefix of output files. See [Output files](interpreting-output.md#output-files) and [Output file formats](interpreting-output.md#output-file-formats) for more details.
...
**`-f <INT=37>`**
Number of bits for bloom filter; 0 to disable. This bloom filter is used to filter out singleton k-mers when counting all k-mers. It takes 2^(INT-3) bytes of memory. A proper setting saves memory. `-f37` is recommended for human assembly. For small genomes, use `-f0` to disable the initial bloom filter which takes 16GB memory at the beginning. For genomes much larger than human, applying `-f38` or even `-f39` is preferred to save memory on k-mer counting.
...
**`--min-hist-cnt <INT=5>`**
When analyzing the k-mer spectrum, ignore counts below `INT`. For very low coverage of HiFi data, set smaller value for this option. See [issue 45](https://github.com/chhylp123/hifiasm/issues/49) for example.
...
**`--hom-cov <INT>`**
Homozygous read coverage inferred automatically in default. This option affects different types of outputs, including Hi-C phased assembly and HiFi-only assembly. For more details, see [How can I tweak parameters to improve Hi-C integrated assembly?](faq.md#how-can-i-tweak-parameters-to-improve-hi-c-integrated-assembly), [Why the size of primary assembly or partially phased assembly is much larger than the estimated genome size?](faq.md#why-the-size-of-primary-assembly-or-partially-phased-assembly-is-much-larger-than-the-estimated-genome-size) and [Hifiasm log interpretation](interpreting-output.md#hifiasm-log-interpretation).
...
**`-1 <FILE>`**
K-mer dump generated by [yak count](https://github.com/lh3/yak) from the paternal/haplotype1 reads.

**`-2 <FILE>`**
K-mer dump generated by [yak count](https://github.com/lh3/yak) from the maternal/haplotype2 reads.
...
**`--t-occ <INT=60>`**
Forcedly remove unitig including `>INT` unexpected haplotype-specific reads without considering graph topology. For more details, see [Why the hamming error rate or the swith error rate of trio-binning assembly is very high?](faq.md#why-the-hamming-error-rate-or-the-swith-error-rate-of-trio-binning-assembly-is-very-high).
...
**`-s <FLOAT=0.55>`**
Similarity threshold for duplicate haplotigs that should be purged. In default, 0.75 for `-l1/-l2`, 0.55 for `-l3`. This option affects both HiFi-only assembly and Hi-C phased assembly. For more details, see [How can I tweak parameters to improve Hi-C integrated assembly?](faq.md#how-can-i-tweak-parameters-to-improve-hi-c-integrated-assembly) and [Why the size of primary assembly or partially phased assembly is much larger than the estimated genome size?](faq.md#why-the-size-of-primary-assembly-or-partially-phased-assembly-is-much-larger-than-the-estimated-genome-size).
