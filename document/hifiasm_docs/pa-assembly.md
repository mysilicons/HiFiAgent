...
In this case, please set `--hom-cov` to homozygous coverage. See [Why the size of primary assembly or partially phased assembly is much larger than the estimated genome size?](faq.md#why-the-size-of-primary-assembly-or-partially-phased-assembly-is-much-larger-than-the-estimated-genome-size) for more details.
...
The primary contigs and the alternate contigs are written to `NA12878.asm.p_ctg.gfa` and `NA12878.asm.a_ctg.gfa`, respectively. For inbred or homozygous genomes, the primary/alternate assemblies can be also produced by `-l0`. Similarly, turning `-s` or `--hom-cov` should be helpful if the primary assembly is too large. See [Why the size of primary assembly or partially phased assembly is much larger than the estimated genome size?](faq.md#why-the-size-of-primary-assembly-or-partially-phased-assembly-is-much-larger-than-the-estimated-genome-size) for more details.
